...
#500
def train_STGCM(adata, hidden_dims=[512, 30], n_epochs=500, lr=0.001, key_added='STGCM',
    ...
    if 'label_CSL' not in adata.obsm.keys(): #添加对比学习标签
    ...
    model = STGCM(hidden_dims = [data.x.shape[1]] + hidden_dims, Conv_type=Conv_type, loss_fn=loss_fn, alpha_l=alpha_l).to(device)
    data = data.to(device)
    print('device:', device)
    ...
    # STGCM_rep = z.to('cpu').detach().numpy()
    STGCM_rep = z.detach().cpu().numpy()
    # print('STGCM_rep:', STGCM_rep)
    adata.obsm[key_added] = STGCM_rep

    if save_loss:
        adata.uns['STGCM_loss'] = loss_list
    # if save_reconstrction:
    #     ReX = out.to('cpu').detach().numpy()
    #     ReX[ReX<0] = 0
    #     adata.layers['STGCM_ReX'] = ReX
