...
def get_feature(adata, deconvolution=False):
    ...
    if isinstance(adata_Vars.X, csc_matrix) or isinstance(adata_Vars.X, csr_matrix):
        ...
        feat = adata_Vars.X[:, ]
    # print('密集矩阵feat.shape:', feat.shape)：（3639，）
...
def permutation(feature):  # 对输入的矩阵进行随机打乱
...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='STGCM', random_seed=2020):
    """\
